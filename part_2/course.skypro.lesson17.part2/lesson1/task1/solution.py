...
def user_by_id(gid: int):
    data = User.query.get(gid)
    schema = UserSchema()
    return schema.dumps(data)
...
@app.route("/tours/<int:gid>", methods=['GET'])
def tour_by_id(gid: int):
    data = Tour.query.get(gid)
    schema = TourSchema()
    return schema.dumps(data)
