...
def guides():
    ...
    offset = detect_offset(page)
    data = Guide.query.limit(page_size).offset(offset).all()
    schema = GuideSchema(many=True)
...
def users():
    page = int(request.args.to_dict().get("page"))
    offset = detect_offset(page)
    data = User.query.limit(page_size).offset(offset).all()
...
def tours():
    page = int(request.args.to_dict().get("page"))
    offset = detect_offset(page)
    data = Tour.query.limit(page_size).offset(offset).all()
    schema = TourSchema(many=True)
    return schema.dumps(data)


def detect_offset(page):
    offset = 0
    if page > 1:
